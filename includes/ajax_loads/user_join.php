...
if($PMLIST['PROC'] == 'id_chk'){
	$already_check = mysqli_query($connect,"SELECT * FROM normal_users WHERE id = '".$PMLIST['ID']."'");
	...
	$already_email_check = mysqli_query($connect,"SELECT * FROM normal_users WHERE email = '".$PMLIST['EMAIL']."'".$where_add);
	...
	$already_email_check = mysqli_query($connect,"SELECT * FROM normal_users WHERE nick = '".$PMLIST['NICKNAME']."'");
	if(mysqli_num_rows($already_email_check) > 0){
		echo "already_used";
		exit;
	}
	...
	if($_REQUEST['shop'] == 'y') $where_add = " AND merchant = 'Y'";
	$already_phone_check = mysqli_query($connect,"SELECT * FROM normal_users WHERE phone = '".$PMLIST['PHONE']."'".$where_add);
	...
	if($PMLIST['PHONE']){
		$already_phone_check = mysqli_query($connect,"SELECT * FROM normal_users WHERE phone = '".$PMLIST['PHONE']."'");
		if(mysqli_num_rows($already_phone_check) > 0){
			echo "already_phone";
			exit;
		}
	...
	mysqli_query($connect,"INSERT INTO normal_users (id,password,name,nick,join_time,join_ip,login_time,login_ip,login_count".$clumn_add.") VALUES ('".$PMLIST['ID']."','".MD5("5taku".$PMLIST['PASS'])."','".$PMLIST['NAME']."','".$PMLIST['NICKNAME']."',NOW(),'".$_SERVER['REMOTE_ADDR']."',NOW(),'".$_SERVER['REMOTE_ADDR']."','1'".$value_add.")");
