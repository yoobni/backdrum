...
if($PMLIST['PROC'] == 'myarticle'){
	...
	if(mysqli_num_rows($already_email_check) > 0){
		...
		mysqli_query($connect,"UPDATE users SET password = '".MD5("danawa".$arr_key)."' WHERE email = '".$PMLIST['EMAIL']."'");
	...
	if(mysqli_num_rows($ip_chk) <= 0){
		$id_chk = mysqli_fetch_array(mysqli_query($connect,"SELECT * FROM users WHERE id = '".$PMLIST['ID']."' AND password = '".MD5("5taku".$PMLIST['PASS'])."' AND status != '1' AND leavedate IS NULL"));
		if($id_chk['idx'] > 0){
