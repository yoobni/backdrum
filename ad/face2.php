...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-107287293-1"></script>
  ...
  setTimeout(function() {
    if ( window.location == 'http://www.backdrum.net/ad/face2.php' ) {
...
<body>
  ...
  <?php
  $myfile = fopen("face2.txt", "r") or die("Unable to open file!");
  $count = fread($myfile, 5);
  $count = $count + 1;
  fclose($myfile);

  $openfile = fopen("face2.txt", "w") or die("Unable to open file!");
